                        ...
                        {r.people.map(p => {
                          ...
                          return (
                            <li key={p.assignment.id} className="flex flex-wrap items-center gap-2 py-1.5 text-sm">
                              ...
                              {canTask && isOnline && <Button size="sm" variant="ghost" className="h-9 sm:h-7 text-xs" title={`Dispatch a task to ${r.position.tactical_callsign || r.position.name}`} onClick={() => { setDispatchFor(r.position.id); setDispatchOpen(true); }}><ListTodo /> Task</Button>}
                              {canRecord && actions.length > 0 && (
                                <span className="ml-auto flex gap-1">
                                  {actions.map(a => { const Icon = ICONS[a.status]; return <Button key={a.status} size="sm" variant={a.primary ? 'default' : 'outline'} className="h-9 sm:h-7 px-2 text-xs" onClick={() => record(p.assignment, a.status)} loading={busyId === p.assignment.id} title={`Record on behalf of ${p.user?.call_sign ?? 'operator'}`}><Icon /> {a.label}</Button>; })}
                                </span>
                              )}
                            </li>
                          );
