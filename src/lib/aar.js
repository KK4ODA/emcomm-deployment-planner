 ...
 */
import { ACTIVITY_TYPES } from './operations';
...
export function aarMarkdown({ deployment, summary, feedback, lessons, usersById, planChanges = [] }) {
  ...
  lines.push('');
  lines.push(`_Hours by type: ${Object.entries(ACTIVITY_TYPES).map(([k, v]) => `${v}`).slice(0, 0).join('')}generated by EmComm Planner on ${new Date().toLocaleDateString()}_`);
  return lines.join('\n');
}
