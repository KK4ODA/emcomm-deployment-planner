...
export const DEFAULT_PREFS = Object.freeze({ push: true, email: false, sms: false });

/** Fill gaps in a stored prefs object. */
export function normalizePrefs(raw) {
  const p = raw && typeof raw === 'object' ? raw : {};
  return { push: p.push !== false, email: p.email === true, sms: p.sms === true };
}
 ...
 * @param {{ status?: { push?: { available?: boolean }, email?: { available?: boolean }, sms?: { available?: boolean } }|null, pushSupported?: boolean, permission?: string, phone?: string|null }} ctx
 * @returns {{ ok: boolean, reason?: string }}
 */
export function channelAvailability(channel, { status = null, pushSupported = true, permission = 'default', phone = null } = {}) {
  if (channel === 'push') {
