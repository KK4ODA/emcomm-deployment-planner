...
import { ROLES, hasPermission, canCreate, canEdit, canDelete, getRoleLabel, getRoleDescription } from './permissions.jsx';
...
describe('role permissions', () => {
  ...
  it('viewer and pending are read-only', () => {
    for (const role of [ROLES.VIEWER, ROLES.PENDING]) {
      for (const entity of ['deployment', 'location', 'category', 'item', 'task', 'template']) {
  ...
  it('rejects unknown roles, permissions and entities', () => {
    ...
    expect(hasPermission('admin', 'NOT_A_PERMISSION')).toBeFalsy();
    expect(canCreate('admin', 'spaceship')).toBeFalsy();
  });
