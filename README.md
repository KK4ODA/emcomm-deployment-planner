...
| Communications | Channel library per ARES group (ICS-217A shape), exportable to and importable from other groups as a file; per-deployment plan with primary / alternate / contingency roles and Condition 1 / 2 / 3 ladder; ICS 205 PDF and CHIRP CSV generated from it; publish with a change note |
| Operations | Check in / On position / Check out from the packet (offline-capable); Operations board with who is missing, arriving, on station; activity log; hours recorded automatically, rolled up per operator and per month for the group (ARRL report buckets) with CSV |
...
| APRS | Through Graywolf: heard stations on the net control board and site map, check-in over APRS from any radio, notifications as APRS messages, sites exported as APRS objects for Emcomm Objects |
...
Two worked tutorials with screenshots, a marathon and a Winter Field Day,
are in [docs/tutorials](docs/tutorials).
