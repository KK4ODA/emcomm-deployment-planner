// Tutorial: planning the Publix Atlanta Marathon in EmComm Planner.
// Facts come from the DeKalb ARES 2026 assignment sheet, frequency plan, ARES map KML,
// HRO handbook and SAG net tactics; volunteer names and phone numbers are not used.
module.exports = {
  file: 'Tutorial - Publix Atlanta Marathon',
  title: 'Planning the Publix Atlanta Marathon',
  subtitle: 'A step-by-step walk through EmComm Planner using a real public-service event: 36 positions, two voice nets, APRS tracking and a downtown course.',
  titleLines: [
  ...
  sections: [
    {
      key: 'intro', h: 'What you will build', blocks: [
        ...
        { table: { columns: ['Item', 'In this scenario'], widths: [0.28, 0.72], rows: [
    ...
    {
      key: 'create', h: 'Create the deployment', step: true, blocks: [
        ...
        { do: [
          'Open **Deployments** in the left menu and click **New deployment**.',
          'Fill in the form as in the table below. Pick **Public service event** as the kind.',
          'Expand **Served agency and authorization** and fill in the agency, the requesting official and the tasking reference.',
          'Click **Create deployment**. The new event appears in the deployment switcher at the top of every page; select it there.',
        ] },
        { enter: { columns: ['Field', 'Value'], widths: [0.3, 0.7], rows: [
    ...
    },
    {
      key: 'map', h: 'Import the course map', step: true, blocks: [
        ...
        { do: [
          ...
          'Click **Add layer**. Switch to the **Map** tab to see the course, the SAG approach routes and the pickup points under your sites.',
        ] },
        { img: 'pam-05-map-layers', caption: 'The Map layers dialog after choosing the KML: it reports what it found before you add it.' },
        { img: 'pam-06-sites-map', caption: 'Sites, Map tab: the course layer with the aid stations and net control pinned on top. The map is what operators see on their packets.' },
    ...
    {
      key: 'sites', h: 'Add the sites', step: true, blocks: [
        ...
        { do: [
          ...
          'Type the name, pick a type, and paste the address or "latitude, longitude" into **Location**. Click the map to drop the pin exactly where the operator should stand.',
          'Fill in **Parking**, **Arrival / where to report** and **Access / credentials**. Save.',
          'Repeat for the other sites. The **Order** field controls the sequence on the packet map and in lists; number the aid stations by mile.',
        ] },
        { enter: { columns: ['Field', 'Aid station mile 20'], widths: [0.3, 0.7], rows: [
        ...
        { img: 'pam-04-site-form', caption: 'Edit site for Aid station mile 20. The map opens on the pin; click it to move the pin.' },
        { img: 'pam-03-sites', caption: 'The Sites list after all 19 sites are in: net control, the warehouse, the finish area, medical, the drop-off, the hospital for reference, and 13 aid stations.' },
        { table: { columns: ['Site', 'Type', 'Why it is a site'], widths: [0.34, 0.24, 0.42], rows: [
        ...
        { why: 'Aid station operators report straight to their station, never to volunteer check-in. Putting that in the arrival note of every aid site is what stops thirty people asking the same question.' },
      ],
    },
    {
      key: 'positions', h: 'Create the positions', step: true, blocks: [
        ...
        { h2: 'Create several at once', do: [
          'Open **Staffing** and click **Create several**.',
          'Enter the patterns from the table. `{n}` is replaced by each number, so AID MILE {n} with the numbers 2, 4, 6 becomes AID MILE 2, AID MILE 4 and AID MILE 6 with tactical calls AID 2, AID 4 and AID 6.',
          'Set the type to **Aid / hydration station**, the net to **RACE**, add the requirements and one shift (05:15 to 14:30 is a safe default; you will trim each station in a moment).',
          'Click **Save as scheme** so next year the position form fills these defaults as you type the name. Then create.',
        ] },
        { enter: { columns: ['Field', 'Value'], widths: [0.3, 0.7], rows: [
        ...
        { img: 'pam-09-create-several', caption: 'Create several positions: the preview at the bottom lists the 13 positions before anything is saved.' },
        { h2: 'Tune each position', p: 'Open a position with the pencil icon to set its site, its report time and its briefing notes. AID MILE 12 is the example: it sits on the stadium ramp where handhelds cannot reach the repeaters, so it gets a cross-band requirement and a note.' },
        { do: [
          'Click the pencil on **AID MILE 12**.',
          'Set **Site** to Aid station mile 12 and **Reports to** NCS RACE (create the net control positions first if you have not; see the table below).',
          'Adjust the shift to the real window: 05:30 to 11:45 for mile 12; aid stations close roughly 30 minutes after the last runner passes.',
          'Add **Cross-band repeat** as a nice-to-have requirement and write the briefing note. Save.',
        ] },
        { img: 'pam-08-position-form', caption: 'Edit position for AID MILE 12: type, site, net, supervisor, requirements, the shift with its muster time, and briefing notes that print on the packet.' },
        { h2: 'The rest of the roster', table: { columns: ['Positions', 'Type', 'Net', 'Site', 'Shift', 'Requirements'], widths: [0.24, 0.14, 0.08, 0.2, 0.14, 0.2], rows: [
        ...
        { img: 'pam-07-staffing', caption: 'Staffing after all 36 positions exist: grouped by site, with the requirements as chips and every shift still open. The header counts 42 seats.' },
        { h2: 'Operational periods', p: 'A one-day race needs a single period. Open **Periods** and confirm "Race day" spans 04:00 to 15:00; the ICS forms and the comms plan are scoped to it.' },
        { img: 'pam-10-periods', caption: 'Operational periods: one is enough for a single-day event.' },
    ...
    {
      key: 'channels', h: 'Build the channel library', step: true, blocks: [
        ...
        { do: [
          'Open **Channels** and click **Channel** for each row in the table. The form suggests the transmit frequency from the standard offset when you type the receive frequency.',
          'For the phone number choose the kind **Phone number**; for APRS choose **Digital / gateway** with the mode APRS.',
          'Put anything an operator must know in **Remarks**; it prints on the ICS 205.',
        ] },
        { enter: { columns: ['Channel', 'Kind', 'RX / TX / tone', 'Owner', 'Remarks'], widths: [0.26, 0.14, 0.24, 0.1, 0.26], rows: [
        ...
        { img: 'pam-12-channels', caption: 'The channel library. Export hands it to another group as a file; Import takes theirs.' },
        { why: 'In 2026 the frequency plan travelled as a spreadsheet tab and one volunteer\'s whole reply to the recruiting thread was "I\'ll need a frequency list". Here the list is on every packet and in the CHIRP file automatically.' },
      ],
    },
    {
      key: 'comms', h: 'Write the communications plan', step: true, blocks: [
        ...
        { do: [
          'Open **Comms plan** and click **Create plan** if there is none.',
          'Click **Add channels** and pick the library rows. For each row set the condition, the role, the function (Tactical, Data, Phone) and the **net** it belongs to (RACE or SAG). Rows without a net print on every packet.',
          'Give the primaries channel numbers 1 to 7 so the CHIRP file and the ICS 205 agree with what people programmed.',
          'Write the **special instructions** and your name as preparer, then **Save details**. Watch **Plan check** on the right turn green.',
        ] },
        { enter: { columns: ['Condition', 'Net', 'Primary', 'Alternate', 'Contingency / Emergency'], widths: [0.16, 0.1, 0.26, 0.24, 0.24], rows: [
        ...
        { img: 'pam-13-comms-plan', caption: 'Condition 1 of the plan: each row carries role, condition, function, net, assignment, channel number and remarks. Plan check on the right confirms a primary, an alternate and a repeaters-down path exist.' },
        { img: 'pam-13-comms-plan-3', caption: 'Condition 3 and the special instructions block. "Program every channel even if it is not yours" is the sentence that saves the day when net control moves a station.' },
    ...
    {
      key: 'staff', h: 'Staff the positions', step: true, blocks: [
        ...
        { do: [
          'On **Staffing**, click the shift chip under **AID MILE 20**.',
          'Read the candidate list. Green means the profile meets every requirement; amber lines say what is missing ("Profile does not say: 6 h independent power") so you can ask before race day, not at the aid station.',
          'Click **Offer** to ask the operator (they get a notification and answer from My Assignments) or **Assign as confirmed** when you already have their yes.',
          'For shifts nobody has claimed, click **Notify N qualified**: every free operator whose profile fits is told the shift is open.',
        ] },
        { img: 'pam-11-assign', caption: 'The assign dialog for AID MILE 20: needs at the top, who is on the shift, and who could take it with the reasons for the ranking.' },
        { h2: 'What the operator sees', p: 'Operators answer offers and pick up open shifts from **My Assignments**. Shifts they do not qualify for, or that overlap one they already hold, say so instead of hiding.' },
        { img: 'pam-19-my-assignments', caption: 'My Assignments for an operator holding AID MILE 20: the accepted position, then open shifts with the reasons they cannot take them.' },
        { why: 'The 2026 marathon needed three escalating appeals over five weeks. With open shifts visible and notifications targeted at qualified people, the coordinator sees exactly which seats are short instead of counting replies.' },
      ],
    },
    {
      key: 'gear', h: 'Equipment kits and setup tasks', step: true, blocks: [
        ...
        { do: [
          'Open **Dashboard**, click **Category** and create "Aid station kit", "SAG vehicle kit" and "Net control kit".',
          'Click **Item** for each piece of gear, choose the category, the site it belongs to, a quantity and a priority. **Assign** it to the operator who brings it once you know.',
          'Add tasks on the site cards: "Confirm MACC roof antenna and feedline" due 26 February, "Cross-band repeater test at the stadium ramp" due 27 February, "Confirm vehicle types with ATC" due 24 February.',
        ] },
        { img: 'pam-16-dashboard', caption: 'Dashboard with the aid station kit: essentials in red until someone is assigned to bring them; site readiness on the right.' },
    ...
    {
      key: 'ready', h: 'Check readiness', step: true, blocks: [
        ...
        { img: 'pam-15-readiness', caption: 'Readiness for the marathon before staffing: open slots, an unpublished plan, essentials nobody brings, and the safety checklist not yet started. Green lines are what is already right.' },
        { bullets: [
    ...
    {
      key: 'publish', h: 'Publish the plan', step: true, blocks: [
        ...
        { do: [
          'On **Staffing** or **Comms plan**, click **Publish plan**.',
          'Read the list of changed positions. Add a **note to operators** if there is something everyone should know (weather, a closed ramp).',
          'Tick **Notify everyone assigned** only for whole-event notes; otherwise leave it off so unaffected operators are not woken.',
          'Click **Publish**.',
        ] },
        { img: 'pam-14-publish', caption: 'Publish plan: the first version lists every position; later versions list only what changed since the last one.' },
        { h2: 'The operator\'s packet', p: 'The packet is one page per assignment with everything needed to show up and get on the air: tactical call, report time, the primary frequency, the site with its map and notes, the full frequency list by condition, what to bring and who to report to. It works offline once opened.' },
        { img: 'pam-20-packet', caption: 'My packet for AID MILE 20: the call for the day, report time, site, primary channel and net control, the course map, and Check in.' },
        { img: 'pam-20-packet-2', caption: 'Further down the packet: parking, arrival and access notes from the site, the frequency plan by condition, and the briefing notes from the position.' },
        { why: 'In 2026 the brief went out three times to everyone, each edition amending the last, with "we apologize for any changes". Publishing per position means an operator whose packet did not change never hears about the change at all.' },
      ],
    },
    {
    ...
    {
      key: 'after', h: 'After the event', step: true, blocks: [
        ...
        { img: 'pam-21-after-action', caption: 'After action: the operator form. Planners see the assembled record above it with Copy draft and Download draft for the written AAR.' },
