// Tutorial: Winter Field Day as a field exercise in EmComm Planner.
// Facts come from the DeKalb ARES WFD objectives document, the 2026 log header
// (WD5EMA, class 2O, section GA) and the Winter Field Day rules.
module.exports = {
  file: 'Tutorial - Winter Field Day',
  title: 'Running Winter Field Day',
  subtitle: 'A step-by-step walk through EmComm Planner for a field exercise: stations instead of aid stations, operating periods, objectives, battery power and a safety checklist.',
  titleLines: [
  ...
  sections: [
    ...
    },
    {
      key: 'create', h: 'Create the deployment', step: true, blocks: [
        ...
        { do: [
          'Open **Deployments** and click **New deployment**.',
          'Fill in the form from the table. Start is the beginning of setup, end is the end of teardown, not the contest period.',
          'Under **Served agency and authorization**, put the club as the agency and the rules and entry class in the tasking reference.',
          'Create it and pick it in the deployment switcher at the top.',
        ] },
        ...
        { img: 'wfd-02-new-deployment', caption: 'New deployment for Winter Field Day. The kind drives the hours category; everything else is the same form.' },
      ],
    },
    {
      key: 'periods', h: 'Define the operating periods', step: true, blocks: [
        ...
        { do: [
          'Open **Staffing** and click **Periods**.',
          'Click **Add period** for each row in the table.',
        ] },
        { enter: { columns: ['Label', 'From', 'To'], widths: [0.4, 0.3, 0.3], rows: [
        ...
        { img: 'wfd-07-periods', caption: 'Operational periods for the weekend. Each shift you create next is attached to one of them.' },
        { why: 'The 2026 rules run the contest 1600 UTC Saturday to 2159 UTC Sunday, which is 11:00 to 16:59 local. Setup may start the Friday but cumulative setup time is capped; putting setup in its own period keeps the record honest.' },
      ],
    },
    {
      key: 'sites', h: 'Add the site and the backup', step: true, blocks: [
        ...
        { do: [
          'Open **Sites**, click **Add site**, and enter the campground from the table. Click the map on the group site to place the pin.',
          'Add the backup site the same way, with a note explaining when it is used.',
        ] },
        { enter: { columns: ['Field', 'Stone Mountain Park campground, group site'], widths: [0.3, 0.7], rows: [
        ...
        { img: 'wfd-04-site-form', caption: 'Edit site for the campground. The arrival and access notes are exactly what a first-time operator asks about.' },
        { img: 'wfd-03-sites', caption: 'Sites: the campground and the backup lot. The backup note records that simplex from the lot to the mountain failed at 50 W in the 2026 drill.' },
      ],
    },
    {
      key: 'stations', h: 'Create the stations and crews', step: true, blocks: [
        ...
        { do: [
          'On **Staffing**, click **Position** and create **HF station** from the table: type Field station, tactical call WFD HF, headcount 2, net TALK-IN, reports to the station captain.',
          'Add one shift per operating slot: Sat 11:00-15:00, 15:00-19:00, 19:00-23:00; Sun 07:00-11:00, 11:00-17:00. Attach each shift to its period; set muster 15 minutes before the slot.',
          'Repeat for the other rows. Rovers have no site; leave **Site** empty for mobile positions.',
        ] },
        { enter: { columns: ['Position', 'Tactical', 'Type', 'Seats', 'Shifts', 'Requirements'], widths: [0.24, 0.12, 0.14, 0.08, 0.22, 0.2], rows: [
        ...
        { img: 'wfd-06-position-form', caption: 'Edit position for the HF station: two seats per slot, the Technician-or-higher requirement (Technicians operate HF under the control operator), and the slots attached to their periods.' },
        { img: 'wfd-05-staffing', caption: 'Staffing with every station and crew: 10 positions, 18 shifts, 29 seats. The captain and the safety officer span the whole weekend.' },
    ...
    },
    {
      key: 'comms', h: 'Talk-in and the rover paths', step: true, blocks: [
        ...
        { do: [
          'On **Channels**, add the rows from the table (the repeater and 146.550 already exist if you did the marathon tutorial).',
          'On **Comms plan**, choose the **Saturday operating** period, add the channels, and assign the talk-in rows to the net **TALK-IN** so they print on every packet.',
          'Add a Condition 3 row: simplex with a relay through Rover 1 from the mountain road.',
        ] },
        { enter: { columns: ['Channel', 'Kind', 'Details', 'Role in the plan'], widths: [0.3, 0.16, 0.3, 0.24], rows: [
    ...
    {
      key: 'objectives', h: 'Post the objectives', step: true, blocks: [
        ...
        { do: [
          'Open **Objectives** and click **Add objectives**.',
          'Enter each objective with a category (primary, secondary, tertiary), a one-line description and points.',
          'On the day, operators click **I will take this**, then **Done**; the captain can mark done or drop from the same list.',
        ] },
        { enter: { columns: ['Objective', 'Category', 'Points'], widths: [0.66, 0.18, 0.16], rows: [
    ...
    {
      key: 'gear', h: 'Equipment, power and assets', step: true, blocks: [
        ...
        { do: [
          'On **Dashboard**, create the categories Stations, Power, Shelter and comfort, Safety, and add the items. Mark radios, antennas, batteries, the canopy, the heater and the first aid kit **essential**.',
          '**Assign** each item to the member who brings it; Readiness warns about essentials nobody is bringing.',
          'On **Assets**, add the club gear once: the IC-7300, the two 100 Ah LiFePO4 batteries, the push-up mast, the 2 m Yagi. Before the event the person who collects them clicks **I have it**; on site, **On site at**; afterwards **Returned**.',
        ] },
        { img: 'wfd-11-dashboard', caption: 'Dashboard for WFD: the Stations category with the club IC-7300, second HF rig, antennas and logging laptops, all still unassigned.' },
        { img: 'wfd-12-assets', caption: 'Assets: the group inventory with custody state. Mark all returned closes the weekend.' },
    ...
    {
      key: 'staff', h: 'Sign people up', step: true, blocks: [
        ...
        { do: [
          'On **Staffing**, click a slot under **HF station**. The dialog lists who could take it, ranked by profile match.',
          '**Offer** it or **Assign as confirmed**. Two seats per slot means two people can hold it.',
          'For empty slots the day before, click **Notify N qualified**.',
        ] },
        { img: 'wfd-08-assign', caption: 'Assign dialog for the first HF slot: two seats, HF voice and licence requirements, and the candidates with their match.' },
        { h2: 'Publish', p: 'Publish the plan once the slots and the site are settled. Everyone assigned gets a packet with the campground notes, the talk-in frequency and their slot. A weather move to the backup site is a site change and a new publication; only the operators whose packet changed are notified.' },
        { img: 'wfd-15-publish', caption: 'Publish plan for WFD. The note to operators is the place for "Bring sleeping bags rated for 20 F".' },
      ],
    },
    {
      key: 'day', h: 'On the weekend', step: true, blocks: [
        { h2: 'Safety officer first', p: 'Under **Safety**, start the standard checklist Saturday morning, walk the site, answer each line and sign. It is the ARRL Field Day safety list; fuel, generator and antenna lines all apply here. The signed checklist is part of the record and clears the Readiness warning.' },
        { img: 'wfd-13-safety', caption: 'The safety checklist before the walk-around. Sign and lock when every line is OK or N/A.' },
        { h2: 'The operator\'s packet', p: 'Each operator opens **My packet** on the way out: report time, the station and its tactical call, the talk-in frequency, the campground map and the gate notes. Check in on arrival, On position when the radio is on the air, Check out at the end of the slot. It works with one bar of signal or none.' },
        { img: 'wfd-16-packet', caption: 'My packet for the first HF slot: WFD HF, report 10:45 for an 11:00 start, talk-in on 146.550, the campground pin, and Check in.' },
        { h2: 'The Operations board and readiness', p: 'The captain keeps **Operations** open with the window set to Everything: who is on station at each position and who has not arrived, plus ICS 214 for the log. **Readiness** shows what is still open before the contest clock starts.' },
        { img: 'wfd-17-net-control', caption: 'The Operations board for the weekend, one line per position, worst first.' },
        { img: 'wfd-14-readiness', caption: 'Readiness before the weekend: open seats, the unpublished plan and the unsigned checklist are the three lines to clear.' },
      ],
    },
    {
