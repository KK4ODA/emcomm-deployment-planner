...
- **Map layers** (Sites › Map): import the course KML, GPX or GeoJSON from the served agency, and turn a layer's waypoints into sites in one click. The colours and line widths from the file are kept, and a legend under the map lists the file's folders (Aid Stations, SAG Routes, and so on). Re-import a layer added before September 2026 to pick up its colours.
...
- **Privacy**: what the app stores, who can see it and how the Google Sheet feature handles Google data is written up at emcommplanner.org/privacy (user menu, Privacy policy).
